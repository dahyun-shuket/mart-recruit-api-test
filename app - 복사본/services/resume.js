...
module.exports = class resumeService {
    ...
    static reactlist(userSeq, regions, jobKinds, name, certificate, recruitSeq) {
        return new Promise(function(resolve, reject) {
            try {
                let result = resumeModel.reactlist(userSeq, regions, jobKinds, name, certificate, recruitSeq);
    
                resolve(result);
            } catch (error) {
                logger.writeLog('error', `services/resumeService/reactlist: ${error}`);           
            }
        })
    }
