...
module.exports = class recruitModel {
    ...
    static async reactList(martSeq, active, name, subject, userSeq, userOwn, regions, jobKinds, workingTypes, scrapSeq) {
        try 
        {
            //쿼리
            const query = `
            ...
            if (rows.length > 0) 
                ...
                logger.writeLog('error', `models/recruitModel.reactList: No data found`);           
                return null;
            }                
        } catch (error) {
            logger.writeLog('error', `models/recruitModel.reactList: ${error}`);           
            return null;
        }
    }
